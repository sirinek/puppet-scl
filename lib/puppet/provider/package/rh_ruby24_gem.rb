...
Puppet::Type.type(:package).provide :rh_ruby24_gem, :parent => Puppet::Provider::Package do
  ...
  to incorporate changing to a Software Collection for ruby24"
  ...
  def self.gemlist(options)
    gem_list_command = [command(:scl), 'enable', 'rh-ruby24', '--', 'gem', 'list']
  ...
  def install(useversion = true)
    command = [command(:scl), 'enable', 'rh-ruby24', '--', 'gem', 'install']
  ...
  def uninstall
    command = command = [command(:scl), 'enable', 'rh-ruby24', '--', 'gem', 'uninstall']
